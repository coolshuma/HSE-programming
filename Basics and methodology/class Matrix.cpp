...
public:
    ...
    long long determinant() {  // Count determinant
        if (N != M) {
        ...
        do {
            ...
            long long sign = (count_of_inversions % 2 == 0 ? 1 : -1);

            long long current_member = 1;

            for (int j = 0; j < N; ++j) {
                current_member *= data[permutation[j]][j];
            }

            determinant += sign * current_member;
